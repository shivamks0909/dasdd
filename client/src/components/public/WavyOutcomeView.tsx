...
export function WavyOutcomeView({ status, statusKeyword }: WavyOutcomeViewProps) {
  ...
  const ip    = stats?.ip || params.ip || "—";
  const loiRaw = parseInt(params.loi || "0", 10);

  // Format LOI from URL param (seconds → "Xm Ys")
  ...
  let dateStr = "—";
  if (stats?.endTime) {
    dateStr = new Date(stats.endTime * 1000).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" });
  } else {
      ...
      >
        ...
        <div className="flex flex-wrap justify-center gap-3 w-full max-w-4xl">
          {cards.map(({ label, value }) => (
            ...
            >
              <span
                style={{ color: `${cfg.glow}99`, fontFamily: "'Inter', sans-serif" }}
              ...
              <span
                style={{ fontFamily: "'Inter', sans-serif" }}
                className="text-base font-black text-white tracking-wide break-all"
              >
                {isLoading ? "…" : value}
              </span>
            </div>
        ...
        <a
          ...
          style={{
            background: "rgba(0,0,0,0.5)",
            border: `1px solid ${cfg.glow}66`,
            boxShadow: `0 0 24px ${cfg.glow}33`,
          }}
        >
          Visit Site →
        </a>
